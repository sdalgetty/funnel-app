...
async function deleteTestForecastModels() {
  ...
  try {
    ...
    const { data: authUserData, error: authError } = await supabase.auth.admin.getUserByEmail(TEST_USER_EMAIL);
    if (authError || !authUserData?.user) {
      console.error('❌ Error getting auth user:', authError?.message || 'User not found');
      return;
    }
